...
plugins=(git colored-man-pages sudo zsh-autosuggestions command-not-found)
...
fi_replace()
{
    escape1=$(echo "$1" | sed 's/\//\\\//')
    escape2=$(echo "$2" | sed 's/\//\\\//')
    find . -type f -exec sed -i "s/$escape1/$escape2/g" {} \;
}
