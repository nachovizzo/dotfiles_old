#!/usr/bin/env python3
# Taken from git@github.com:mseitzer/gpu-monitor.git
...
NVIDIASMI_CMD_XML = 'nvidia-smi -q -x'
NVIDIASMI_CMD = 'nvidia-smi'

# Command for running nvidia-smi remotely
REMOTE_NVIDIASMI_CMD_XML = '{} {}'.format(SSH_CMD, NVIDIASMI_CMD_XML)
...
def run_nvidiasmi_local():
    res = run_command(NVIDIASMI_CMD_XML)
...
def run_nvidiasmi_remote(server,
    ...
    return str(run_command(cmd)).replace("\\n", '\n').replace('\'', '')


def run_nvidiasmi_remote_xml(server, ssh_timeout, cmd_timeout):
    cmd = REMOTE_NVIDIASMI_CMD_XML.format(server=server,
                                          ssh_timeout=ssh_timeout,
                                          cmd_timeout=cmd_timeout)
    res = run_command(cmd)
    return ET.fromstring(res) if res is not None else None
...
def main(argv):
    ...
    for server in args.servers:
        if server == '.' or server == 'localhost' or server == '127.0.0.1':
            ...
            run_nvidiasmi = partial(run_nvidiasmi_remote_xml,
                                    server=server,
